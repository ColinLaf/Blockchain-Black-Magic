...
def getMap():
    ...
    canvas = Canvas(root, width = 1000, height = 575)
    canvas.pack()
    img = PhotoImage(file= "graphv3.ppm") #ppm files are the only ones that Tkinter is able to open (To my knowledge)
    img = img.subsample(2,2) #Image was too large, so this command is used in order to change the size
    canvas.create_image(0,0, anchor=NW, image=img)
    root.title("Map")

    Message(root, text='VALERIAN SPRIGS is at TVE, HOOP, SLH, ESTA, VARS', font=('Times', 12, 'bold'), width=450).pack(anchor='nw') #Message blocks are packed in the direction of the anchor
    Message(root, text='ALIHOSTY LEAVES is at MBRC, LIBR, RUC', font=('Times', 12, 'bold'), width=450).pack(anchor='nw')
    Message(root, text='BILLYWIG STRINGS is at SWH, CRAW', font=('Times', 12, 'bold'), width=450).pack(anchor='nw')
    Message(root, text='PEARL DUST is at RMH, USVC, FNDH, MNTO', font=('Times', 12, 'bold'), width=450).pack(anchor='nw')
    Message(root, text='DITTANY is at PRSC, LEWS', font=('Times', 12, 'bold'), width=450).pack(anchor='nw')
    Button(root, text = 'Main Menu', bd=5, pady=4, command =lambda : moveMenu(root, 0)).pack(anchor = 'nw', side = 'left')
    Button(root, text = 'Security Log', bd=5, pady=4, command =lambda : moveMenu(root, 2)).pack(anchor = 'nw', side = 'left')
    Button(root, text = 'Potion Book', bd=5, pady=4, command =lambda : moveMenu(root, 3)).pack(anchor = 'nw', side = 'left')
...
def checkBlockchain():
    ...
    myList = Listbox(frame, yscrollcommand = scrollbar.set, height=20, width=240, bg='black', fg='lightgreen')
    ...
    Button(root, text = 'Main Menu', bd=5, command =lambda : moveMenu(root, 0)).pack(anchor='nw', side = 'left')
    Button(root, text = 'Potion Book', bd=5, command =lambda : moveMenu(root, 3)).pack(anchor = 'nw', side = 'left')
    Button(root, text= "Open Map", bd = 5, command = lambda : moveMenu(root, 1)).pack(anchor='nw', side = 'left')
...
def potionBook():
    ...
    canvas.create_image(0,0, anchor=NW, image=img)
    btn = Button(root, text='>', command = lambda : moveMenu(root, 4))  #(command = lamda :) is used to send commands with arguments
    canvas.create_window( 960, 50, window = btn)
    btn = Button(root, text='Main Menu', command = lambda : moveMenu(root, 0))
    canvas.create_window( 512, 710, window = btn)
    btn = Button(root, text='Security Logs', command = lambda : moveMenu(root, 1))
    canvas.create_window( 432, 710, window = btn)
    btn = Button(root, text='Open Map', command = lambda : moveMenu(root, 2))
    canvas.create_window( 584, 710, window = btn)
    root.mainloop()
...
def page2():
    ...
    canvas = Canvas(root, width = 1024, height = 720) #Canvas' are used for backgrounds and images being placed inside of the window
    canvas.pack()
    img = PhotoImage(file= "page2.ppm")
    canvas.create_image(0,0, anchor=NW, image=img) #This command places the images and other things within the canvas
    btn = Button(root, text='<', command = lambda : moveMenu(root, 3))
    canvas.create_window( 20,50, window = btn) #Without this command the buttons are placed beneath the background rather than within
    btn = Button(root, text='Main Menu', command = lambda : moveMenu(root, 0))
    canvas.create_window( 512, 710, window = btn)
    btn = Button(root, text='Security Logs', command = lambda : moveMenu(root, 1))
    canvas.create_window( 432, 710, window = btn)
